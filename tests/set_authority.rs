...
fn set_authority_ix(
    ...
    let mut data = vec![IX_SET_AUTHORITY];
...
#[test]
fn test_set_authority_happy_path() {
    ...
    let ix = set_authority_ix(program_id, token_kp.pubkey(), authority.pubkey(), &new_authority.pubkey());
    let result = send(&mut svm, ix, &payer, &[&payer, &authority]);
    print_logs("set_authority_happy_path", &result);
    result.unwrap();

    let acct = svm.get_account(&token_kp.pubkey()).unwrap();
    assert_eq!(&acct.data[TA_AUTHORITY..TA_AUTHORITY + 32], &new_authority.pubkey().to_bytes());
}

#[test]
fn test_set_authority_wrong_authority() {
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let mint = Pubkey::new_unique();
    ...
    let ix = set_authority_ix(program_id, token_kp.pubkey(), wrong_authority.pubkey(), &new_authority.pubkey());
    let result = send(&mut svm, ix, &payer, &[&payer, &wrong_authority]);
    print_logs("set_authority_wrong_authority", &result);
    assert_eq!(result.unwrap_err().err, custom_err(ERR_AUTHORITY_MISMATCH));
}

#[test]
fn test_set_authority_not_signer() {
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let mint = Pubkey::new_unique();
    ...
    let mut data = vec![IX_SET_AUTHORITY];
    data.extend_from_slice(&new_authority.pubkey().to_bytes());
    ...
    print_logs("set_authority_not_signer", &result);
    assert_eq!(result.unwrap_err().err, custom_err(ERR_NOT_SIGNER));
}

#[test]
fn test_set_authority_not_writable() {
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let mint = Pubkey::new_unique();
    ...
    let result = send(&mut svm, ix, &payer, &[&payer, &authority]);
    print_logs("set_authority_not_writable", &result);
    assert_eq!(result.unwrap_err().err, custom_err(ERR_NOT_WRITABLE));
}

#[test]
fn test_set_authority_wrong_acct_count() {
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let mint = Pubkey::new_unique();
    ...
    let result = send(&mut svm, ix, &payer, &[&payer]);
    print_logs("set_authority_wrong_acct_count", &result);
    assert_eq!(result.unwrap_err().err, custom_err(ERR_WRONG_ACCT_COUNT));
}

#[test]
fn test_set_authority_wrong_token_size() {
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let authority = Keypair::new();
    ...
    let ix = set_authority_ix(program_id, token_kp.pubkey(), authority.pubkey(), &new_authority.pubkey());
    let result = send(&mut svm, ix, &payer, &[&payer, &authority]);
    print_logs("set_authority_wrong_token_size", &result);
    assert_eq!(result.unwrap_err().err, custom_err(ERR_WRONG_ACCT_SIZE));
}

#[test]
fn test_set_authority_wrong_ix_data_len() {
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let mint = Pubkey::new_unique();
    ...
    // only disc byte, missing new_authority (should be 33 bytes)
    let ix = Instruction::new_with_bytes(
        program_id,
        &[IX_SET_AUTHORITY],
        vec![
    ...
    let result = send(&mut svm, ix, &payer, &[&payer, &authority]);
    print_logs("set_authority_wrong_ix_data_len", &result);
    assert_eq!(result.unwrap_err().err, custom_err(ERR_INVALID_IX));
}

#[test]
fn test_set_authority_new_authority_can_sign() {
    // verify the new authority can subsequently update authority again
    let (mut svm, program_id) = setup();
    let payer = Keypair::new();
    let token_kp = Keypair::new();
    let mint = Pubkey::new_unique();
    let authority = Keypair::new();
    let new_authority = Keypair::new();
    let final_authority = Keypair::new();
    ...
    // first transfer
    let ix = set_authority_ix(program_id, token_kp.pubkey(), authority.pubkey(), &new_authority.pubkey());
    send(&mut svm, ix, &payer, &[&payer, &authority]).unwrap();

    // second transfer with new authority
    let ix = set_authority_ix(program_id, token_kp.pubkey(), new_authority.pubkey(), &final_authority.pubkey());
    let result = send(&mut svm, ix, &payer, &[&payer, &new_authority]);
    print_logs("set_authority_new_authority_can_sign", &result);
    result.unwrap();

    let acct = svm.get_account(&token_kp.pubkey()).unwrap();
    assert_eq!(&acct.data[TA_AUTHORITY..TA_AUTHORITY + 32], &final_authority.pubkey().to_bytes());
}
