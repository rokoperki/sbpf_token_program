...
fn test_init_mint_happy_path() {
    ...
    svm.set_account(
        mint_kp.pubkey(),
        mint_account(program_id, vec![0u8; MINT_SZ]),
    )
    .unwrap();
    ...
    let ix = make_ix(
        ...
        mint_kp.pubkey(),
        authority.pubkey(),
        init_mint_data(authority.pubkey()),
        true,
        true,
    );
    ...
    assert_eq!(
        &acct.data[MA_AUTHORITY..MA_AUTHORITY + 32],
...
fn test_init_mint_wrong_acct_size() {
    ...
    svm.set_account(mint_kp.pubkey(), mint_account(program_id, vec![0u8; 8]))
        .unwrap();
...
fn test_init_mint_already_initialized() {
    ...
    svm.set_account(mint_kp.pubkey(), mint_account(program_id, data))
        .unwrap();
